...
//		output wire  M_AXI_RREADY
	...
	assign M_AXI_AWADDR	= 'h80000000;
	//AXI 4 write data
	assign M_AXI_WDATA	= {{(C_M_AXI_DATA_WIDTH - PAYLOAD_WIDTH){1'b0}},snake_locations};
...
//    assign M_AXI_AWVALID	= go_signal;
    ...
    snake_game_top_0 snake_top(
        .clk(go_signal), // a synchronization clk from server eventually
        .resetn(aresetn),
        .input_dir(2'b0),// 0 - up, 1 - left, 2 - down, 3 - right  <- input from player
        .snake_x_out(snake_x),
        .snake_y_out(snake_y),
        .size_out(size)
    );
